...
async function fetchDiaries() {
  loading.value = true
  try { const res = await getDiaries({ pageSize, nowPage: currentPage.value, serchTerm: searchTerm.value || undefined }); if (res.code === 200 && res.data) { diaries.value = getList<DiaryDate>(res.data); totalCount.value = res.data.count } } catch { /* */ }
  loading.value = false
}
...
<template>
  <div class="relative min-h-screen">
    <div class="pt-16 sm:pt-24 pb-16 max-w-2xl mx-auto px-6">
      <ScrollReveal>
        <div class="mb-10">
          <p class="text-xs font-bold text-accent-3 uppercase tracking-widest mb-2">Diary</p>
          <h1 class="text-3xl sm:text-4xl font-extrabold text-zinc-800">日记</h1>
          <p class="mt-1.5 text-zinc-400 text-sm" v-if="totalCount >= 0">共 {{ totalCount }} 篇</p>
        </div>
      </ScrollReveal>
      ...
      <div v-else class="space-y-4">
        <ScrollReveal v-for="(d,i) in diaries" :key="d.id" :delay="i*60"><DiaryCard :diary="d" /></ScrollReveal>
      </div>
